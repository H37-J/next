...
// 23396
// 24487
for (let i = 25986; i >= 1; i--) {
    await page.setDefaultNavigationTimeout(0);
    const url = `https://2211.92mj.net/points-14-${i}-1.html`
    await page.goto(url)
...
async function gif(title, allTime, dir, count) {
    ...
    for (let k = 1; k < count; k++) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            try {
                const command = `cd ${title} && ffmpeg -ss ${arr[i]} -t 00:00:02 -i ${k}.mp4 -vf "fps=5,scale=640:-1:flags=lanczos" -c:v gif ${k}_${i}.gif`
                console.log(command)
                const child = exec(command, {stdio: 'pipe'});
                child.stdout.on('data', (data) => {
                    console.log(`stdout: ${data}`);
